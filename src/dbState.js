...
async function runMigration(p) {
  ...
  console.log('🔄 [DB] MySQL connected — migrating JSON fallback data to MySQL...');
  try {
    const data = exportAllForMigration();

    const run = (sql, params = []) => p.execute(sql, params).catch(() => {});
    ...
    for (const r of (data.snipe_cache || [])) {
      await run(
        `INSERT INTO snipe_cache (guild_id, channel_id, content, author_id, author_name, author_avatar, deleted_at) VALUES (?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE content=VALUES(content), author_id=VALUES(author_id), author_name=VALUES(author_name), author_avatar=VALUES(author_avatar), deleted_at=VALUES(deleted_at)`,
    ...
    for (const r of (data.auto_reacts || [])) {
      await run(
        `INSERT INTO auto_reacts (guild_id, user_id, emoji_id, emoji_name, animated) VALUES (?,?,?,?,?) ON DUPLICATE KEY UPDATE emoji_id=VALUES(emoji_id), emoji_name=VALUES(emoji_name), animated=VALUES(animated)`,
...
async function initTables(p) {
  const tables = [
    `CREATE TABLE IF NOT EXISTS guilds (
      guild_id VARCHAR(20) PRIMARY KEY, log_channel_id VARCHAR(20), warn_log_channel_id VARCHAR(20),
      strike_log_channel_id VARCHAR(20), request_log_channel_id VARCHAR(20), ad_warn_log_channel_id VARCHAR(20),
      ad_warn_dm_log_channel_id VARCHAR(20), level_log_channel_id VARCHAR(20), level_xp_channel_id VARCHAR(20),
      leveling_enabled TINYINT(1) DEFAULT 1, staff_updates_channel_id VARCHAR(20), jail_role_id VARCHAR(20),
      muted_role_id VARCHAR(20), command_roles TEXT, ban_request_channel_id VARCHAR(20),
      blacklist_request_channel_id VARCHAR(20), network_ban_request_channel_id VARCHAR(20),
      partnership_request_channel_id VARCHAR(20), is_hub TINYINT(1) DEFAULT 0, hub_guild_id VARCHAR(20),
      break_request_channel_id VARCHAR(20), break_role_id VARCHAR(20), main_break_role_id VARCHAR(20),
      resign_channel_id VARCHAR(20), verified_role_id VARCHAR(20), applications_channel_id VARCHAR(20),
      referral_link TEXT, modmail_test_channel_id VARCHAR(20), pfp_url TEXT, banner_url TEXT,
      network_apply_log_channel_id VARCHAR(20), network_apply_roles TEXT, abuse_log_channel_id VARCHAR(20),
      hub_mod_role_id VARCHAR(20), hub_team_lead_role_id VARCHAR(20), hub_admin_role_id VARCHAR(20),
      hub_owner_role_id VARCHAR(20), github_repo VARCHAR(200), is_staff_server TINYINT(1) DEFAULT 0,
      staff_guild_id VARCHAR(20)
    )`,
    `CREATE TABLE IF NOT EXISTS warns (id INT AUTO_INCREMENT PRIMARY KEY, case_id VARCHAR(20), guild_id VARCHAR(20), user_id VARCHAR(20), moderator_id VARCHAR(20), reason TEXT, created_at INT)`,
    `CREATE TABLE IF NOT EXISTS ad_warns (id INT AUTO_INCREMENT PRIMARY KEY, case_id VARCHAR(20), guild_id VARCHAR(20), user_id VARCHAR(20), moderator_id VARCHAR(20), reason TEXT, message_id VARCHAR(20), message_content TEXT, created_at INT)`,
    `CREATE TABLE IF NOT EXISTS strikes (id INT AUTO_INCREMENT PRIMARY KEY, case_id VARCHAR(20), guild_id VARCHAR(20), user_id VARCHAR(20), moderator_id VARCHAR(20), reason TEXT, created_at INT)`,
    `CREATE TABLE IF NOT EXISTS jailed_users (guild_id VARCHAR(20), user_id VARCHAR(20), original_roles TEXT, jailed_at INT, PRIMARY KEY (guild_id, user_id))`,
    `CREATE TABLE IF NOT EXISTS message_counts (guild_id VARCHAR(20), user_id VARCHAR(20), count INT DEFAULT 0, PRIMARY KEY (guild_id, user_id))`,
    `CREATE TABLE IF NOT EXISTS snipe_cache (guild_id VARCHAR(20), channel_id VARCHAR(20), content TEXT, author_id VARCHAR(20), author_name VARCHAR(200), author_avatar TEXT, deleted_at INT, PRIMARY KEY (guild_id, channel_id))`,
    `CREATE TABLE IF NOT EXISTS balances (guild_id VARCHAR(20), user_id VARCHAR(20), balance INT DEFAULT 0, PRIMARY KEY (guild_id, user_id))`,
    `CREATE TABLE IF NOT EXISTS breaks (id INT AUTO_INCREMENT PRIMARY KEY, guild_id VARCHAR(20), user_id VARCHAR(20), username VARCHAR(200), reason TEXT, started_at INT, end_at INT, saved_roles TEXT)`,
    `CREATE TABLE IF NOT EXISTS applications (id INT AUTO_INCREMENT PRIMARY KEY, guild_id VARCHAR(20), user_id VARCHAR(20), username VARCHAR(200), data TEXT, submitted_at INT)`,
    `CREATE TABLE IF NOT EXISTS levels (guild_id VARCHAR(20), user_id VARCHAR(20), total_xp BIGINT DEFAULT 0, PRIMARY KEY (guild_id, user_id))`,
    `CREATE TABLE IF NOT EXISTS disabled_commands (guild_id VARCHAR(20), command_name VARCHAR(100), PRIMARY KEY (guild_id, command_name))`,
    `CREATE TABLE IF NOT EXISTS bot_blacklist (id INT AUTO_INCREMENT PRIMARY KEY, guild_id VARCHAR(20), user_id VARCHAR(20), moderator_id VARCHAR(20), reason TEXT, created_at INT)`,
    `CREATE TABLE IF NOT EXISTS network_applications (id INT AUTO_INCREMENT PRIMARY KEY, target_guild_id VARCHAR(20), applicant_id VARCHAR(20), applicant_username VARCHAR(200), applicant_avatar TEXT, why TEXT, experience TEXT, timezone VARCHAR(100), age VARCHAR(10), status VARCHAR(20) DEFAULT 'pending', created_at INT)`,
    `CREATE TABLE IF NOT EXISTS ad_channels (guild_id VARCHAR(20), channel_id VARCHAR(20), PRIMARY KEY (guild_id, channel_id))`,
    `CREATE TABLE IF NOT EXISTS ad_posts (id INT AUTO_INCREMENT PRIMARY KEY, guild_id VARCHAR(20), channel_id VARCHAR(20), message_id VARCHAR(20), user_id VARCHAR(20), created_at INT)`,
    `CREATE TABLE IF NOT EXISTS auto_reacts (guild_id VARCHAR(20), user_id VARCHAR(20), emoji_id VARCHAR(20), emoji_name VARCHAR(100) NOT NULL, animated TINYINT(1) DEFAULT 0, ar_expires_at DATETIME NULL, PRIMARY KEY (guild_id, user_id))`,
    `CREATE TABLE IF NOT EXISTS sticky_messages (guild_id VARCHAR(20), channel_id VARCHAR(20), message TEXT NOT NULL, PRIMARY KEY (guild_id, channel_id))`,
    `CREATE TABLE IF NOT EXISTS sticky_channel_state (guild_id VARCHAR(20), channel_id VARCHAR(20), last_message_id VARCHAR(20), PRIMARY KEY (guild_id, channel_id))`,
    `CREATE TABLE IF NOT EXISTS hall_of_shame (guild_id VARCHAR(20), message_id VARCHAR(20), PRIMARY KEY (guild_id, message_id))`,
    `CREATE TABLE IF NOT EXISTS invite_blacklist (guild_id VARCHAR(20), blocked_guild_id VARCHAR(20), added_by VARCHAR(20), added_at INT, PRIMARY KEY (guild_id, blocked_guild_id))`,
    `CREATE TABLE IF NOT EXISTS honeypot_config (guild_id VARCHAR(20) PRIMARY KEY, channel_id VARCHAR(20) NOT NULL, alert_channel_id VARCHAR(20), action VARCHAR(20) NOT NULL DEFAULT 'none', created_at INT NOT NULL, created_by VARCHAR(20) NOT NULL)`,
    `CREATE TABLE IF NOT EXISTS honeypot_triggers (id INT AUTO_INCREMENT PRIMARY KEY, guild_id VARCHAR(20) NOT NULL, user_id VARCHAR(20) NOT NULL, username VARCHAR(200), content_preview TEXT, triggered_at INT NOT NULL, action_taken VARCHAR(20) NOT NULL DEFAULT 'none')`,
  ];
  for (const sql of tables) await p.execute(sql).catch(() => {});
...
async function tryConnect() {
  try {
    ...
    console.warn(`⚠️  [DB] MySQL unavailable — using JSON fallback. (${err.message})`);
    connected = false;
    pool = null;
    return false;
  }
