...
function ApplicationsTab() {
  ...
  const [testAnswerType, setTestAnswerType] = useState('ai');
  ...
  async function sendTestApp(e) {
    ...
    setTestResult(null);
    try {
      const res = await api('/api/admin/test-application', { method: 'POST', body: JSON.stringify({ role: testRole, answerType: testAnswerType }) });
  ...
  return (
    <div>
      ...
      <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 18px', marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-muted)' }}>🧪 Test Application</div>
        <form onSubmit={sendTestApp} style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', borderRadius: 8, overflow: 'hidden', border: '1px solid var(--border)', flexShrink: 0 }}>
            {[{ value: 'ai', label: '🤖 AI Answers' }, { value: 'human', label: '🙋 Human Answers' }].map(opt => (
              <button
                key={opt.value}
                type="button"
                onClick={() => { setTestAnswerType(opt.value); setTestResult(null); }}
                style={{
                  padding: '6px 14px',
                  fontSize: 12,
                  fontWeight: 600,
                  border: 'none',
                  cursor: 'pointer',
                  background: testAnswerType === opt.value ? 'var(--accent)' : 'var(--surface3)',
                  color: testAnswerType === opt.value ? '#fff' : 'var(--text-muted)',
        ...
        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
          {testAnswerType === 'ai'
            ? '🤖 Answers generated by HuggingFace — should score high on AI detection.'
            : '🙋 Casual human-style answers written locally — should score low on AI detection.'}
        </div>
        {testResult && (
          testResult.error
            ? <div style={{ color: 'var(--danger)', fontSize: 13 }}>❌ {testResult.error}</div>
            : <div style={{ color: 'var(--success)', fontSize: 13 }}>✅ {testResult.success}</div>
        )}
      </div>
