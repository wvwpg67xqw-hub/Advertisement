...
router.post(
  '/',
  requireAuth,
  rateLimit('applications', 5, 60 * 60 * 1000), // 5 submissions per hour per IP
  async (req, res) => {
    ...
    // Length guards — prevent abuse / embed flooding
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({ text: `Application ID: ${applicationId} · IP: ${getClientIp(req)}` })
      .setTimestamp();
    ...
    if (!channelId) {
      return res.status(500).json({ error: 'Server misconfigured (missing APPLICATION_CHANNEL_ID)' });
    }
